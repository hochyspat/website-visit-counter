...
@pytest.fixture
def mock_logs() -> list[tuple[str, str]]:
    return [
...
@pytest.fixture
def api_manager(mock_logs: list[tuple[str, str]]) -> ApiManager:
...
def test_total_visits(api_manager: ApiManager) -> None:
    assert api_manager.api_visits_all() == 5


def test_visits_day(api_manager: ApiManager) -> None:
    assert api_manager.api_visits_day("2025-04-07") == 2


def test_visits_month(api_manager: ApiManager) -> None:
    assert api_manager.api_visits_month(r"2025-04-\d{2}") == 4


def test_visits_year(api_manager: ApiManager) -> None:
    assert api_manager.api_visits_year(r"2025-\d{2}-\d{2}") == 5


def test_uniq_visits_day(api_manager: ApiManager) -> None:
    assert api_manager.api_uniq_visits_day("127.0.0.1", "2025-04-07") == 1


def test_uniq_visits_all(api_manager: ApiManager) -> None:
